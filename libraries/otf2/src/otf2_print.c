...
OTF2_CallbackCode print_global_def_location(void *userData,
  ...
  OTF2_PRINT_DEF("Global_def_location(userData=%p, self=%" PRIu64 ", name=%d, locationType=%d, numberOfEvents=%" PRIu64 ", locationGroup=%d)\n",
	 userData,
	 self,
	 name,
	 locationType,
...
OTF2_CallbackCode print_mpi_send(OTF2_LocationRef locationID,
  ...
  snprintf(buffer, STRING_MAX_LEN, "receiver: %d, comm: %x, tag: %d, len: %" PRIu64,
...
OTF2_CallbackCode print_mpi_isend(OTF2_LocationRef locationID,
				  ...
				  uint64_t requestID) {
  char buffer[STRING_MAX_LEN];
  snprintf(buffer, STRING_MAX_LEN, "receiver: %d, comm: %x, tag: %d, len: %" PRIu64 ", req: %p",
...
OTF2_CallbackCode print_mpi_recv(OTF2_LocationRef locationID,
				 ...
				 uint64_t msgLength) {
  char buffer[STRING_MAX_LEN];
  snprintf(buffer, STRING_MAX_LEN, "sender: %d, comm: %x, tag: %d, len: %" PRIu64,
...
OTF2_CallbackCode print_mpi_irecv(OTF2_LocationRef locationID,
				  ...
				  uint64_t requestID) {
  char buffer[STRING_MAX_LEN];
  snprintf(buffer, STRING_MAX_LEN, "sender: %d, comm: %x, tag: %d, len: %" PRIu64 ", req: %p",
...
OTF2_CallbackCode print_mpi_collective_end(OTF2_LocationRef locationID,
  ...
  char buffer[STRING_MAX_LEN];
  snprintf(buffer, STRING_MAX_LEN, "op: %d, comm: %x, root: %d, sent: %" PRIu64 ", recved: %" PRIu64,
...
OTF2_CallbackCode print_omp_task_create(OTF2_LocationRef locationID,
					...
					uint64_t taskID) {
  char buffer[STRING_MAX_LEN];
  snprintf(buffer, STRING_MAX_LEN, "taskID: %" PRIu64, taskID);
...
OTF2_CallbackCode print_non_blocking_collective_complete(
							 ...
							 uint64_t requestID) {
  char buffer[STRING_MAX_LEN];
  snprintf(buffer, STRING_MAX_LEN, "op: %d, comm: %x, root: %d, sent: %" PRIu64 ", recved:%" PRIu64 ", requestID: %p",
