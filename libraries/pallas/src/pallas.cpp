...
const char* Thread::getRegionStringFromEvent(pallas::EventData* e) const {
...
std::map<Token, pallas_duration_t> Thread::getSnapshotViewExact(pallas_timestamp_t start, pallas_timestamp_t end) const {
  ...
  // return a timeprofile between start and end
  // sequences with similar names are fused
...
#endif
	std::string id=s.guessName(this);
