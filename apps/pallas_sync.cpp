...
int sync_events(std::vector<pallas::Thread*>& threads, pallas::Thread* t, uint32_t start_id, uint32_t end_id, thread_token_map& event_map, thread_token_map& event_rev) {
    for (auto* t2 : threads) {
...
void update_loop_tokens(std::vector<pallas::Thread*>& threads,
    ...
    for (auto* t : threads) {
        for (uint32_t logi_id = 0; logi_id < t->loop_id_map.size(); logi_id++) {
            ...
            if (owner_it == thread_it->second.end())
                continue;
...
int sync_loops(std::vector<pallas::Thread*>& threads, pallas::Thread* t, uint32_t start_id, uint32_t end_id, thread_token_map& loop_map, thread_token_map& loop_rev) {
...
void update_sequence_tokens(std::vector<pallas::Thread*>& threads,
    ...
    for (auto* t : threads) {
        for (uint32_t logi_id = 0; logi_id < t->sequence_id_map.size(); logi_id++) {
            ...
            seq.hash = pallas::hash32(reinterpret_cast<const byte*>(seq.tokens.data()), seq.tokens.size() * sizeof(pallas::Token), SEED);
        }
    }
}

int sync_sequences(std::vector<pallas::Thread*>& threads, pallas::Thread* t, uint32_t start_id, uint32_t end_id, thread_token_map& seq_map, thread_token_map& seq_rev) {
    int number_of_swaps = 0;
    for (auto* t2 : threads) {
        // ignore identical threads
        if (t2->id == t->id) {
...
void save_thread_copy(pallas::GlobalArchive* trace, std::vector<pallas::Archive*>& archives, std::vector<pallas::Thread*>& threads, char* save_dir_name) {
    for (auto* t : threads) {
...
int main(int argc, char** argv) {
    ...
    auto base_dir_name = strdup((std::string(trace->dir_name)).c_str());

    auto base_trace_name = strdup((std::string(base_dir_name) + "/" + std::string(trace->trace_name)).c_str());

    auto temp_dir_name = strdup((std::string(base_dir_name) + "_temp").c_str());
    ...
    for (auto const& [string_ref, string] : trace->definitions.strings) {
        ...
        for (auto const& [synced_string_ref, synced_string] : synced_strings) {
    ...
    for (auto const& [region_ref, region] : trace->definitions.regions) {
        if (DEBUG_LEVEL > 0) {
        ...
        for (auto const& [synced_region_ref, synced_region] : synced_regions) {
    ...
    // attributes
    for (auto& [attribute_ref, attribute] : trace->definitions.attributes) {
        attribute.name = string_ref_lookup[attribute.name];
        attribute.description = string_ref_lookup[attribute.description];
    }

    // groups
    for (auto& [group_ref, group] : trace->definitions.groups) {
        group.name = string_ref_lookup[group.name];
    }

    // comms
    for (auto& [comm_ref, comm] : trace->definitions.comms) {
    ...
    while (true) {
        ...
        update_loop_tokens(threads, thread_event_map, update_events, thread_seq_map, update_seqs, thread_loop_map, update_loops, thread_loop_rev, loop_base_tokens);
        ...
        update_sequence_tokens(threads, thread_event_map, update_events, thread_seq_map, update_seqs, thread_loop_map, update_loops, thread_seq_rev, seq_base_tokens);
